...
app.get('/player_info/post', function(req, res) {
	...
	if (player_id != 'Select Player'){
		...
		var query3 = 'SELECT COUNT(*) FROM football_players INNER JOIN football_games ON ' + 
		'football_players.id = ANY(players) WHERE football_players.id = ' + player_id + ';';
		db.task('get-everything', task => {
		    return task.batch([
		        task.any(query1),
		        task.any(query2),
